...
/* If true, interactive renames are done pax-style.  This is only
   useful if rename_flag is set.  */
char pax_rename_flag = false;
...
/* Array of filenames to use in 'pax' mode.  These files are either
   written to an archive or copied.  */
char **pax_file_names = 0;
int num_pax_file_names;
...
/* True if messages like "1 block" should be suppressed.  */
char no_block_message_flag = false;

/* True if we recurse into directories when writing.  */
char directory_recurse_flag = false;

/* True if we can overwrite existing files.  */
char overwrite_existing_files = true;

/* True if we should preserve mode of files.  */
char preserve_mode_flag = true;

/* True if we can cross devices while making archive.  */
char cross_device_flag = true;

/* Current device id.  Used to implement no-device-crossing.  */
unsigned long current_device;

/* Returns true if name is too long for archive format.
   Only used when writing.  */
int (*name_too_long) PARAMS ((char *name));
